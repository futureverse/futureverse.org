...
April 2023, **future** was among the top-0.9% most downloaded
package on CRAN (Figure 1) and there are 265 packages on CRAN that
directly depend on it (Figure 2).  For map-reduce parallelization
packages **future.apply** (top-1.2% most downloaded) and and **furrr**
(top 1.6%), the corresponding number of packages are 116 and 85,
...
via another package, then 85% of all ~19,500 CRAN packages may rely on
...
<center>
<img src="figures/downloads_over_time_on_CRAN.png" alt="A line graph with 'Date' on the horizontal axis and 'Download rates on CRAN (four-week averages)' on the vertical axis. The dates goes from mid 2015 to mid 2023 and the ranks for 0 to 20%. Lines for package 'foreach', 'future', 'future.apply', and 'furrr' are displayed in different colors. The foreach curve is the highest but decreases slowly, whereas the other three are rapidly increasing toward the level of foreach." style="width: 70%;"/>
</center>
<em>
...
has 838 reverse package dependencies on CRAN.  The number of users
