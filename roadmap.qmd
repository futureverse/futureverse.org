...
The below features are part of the bigger **[Resource Specifications]** Project.
...
* {{< bi square-half >}}<span>Memory awareness: Add functions to **parallelly** for querying total, available, and current-process memory - `availableMemory()`, `freeMemory()`, and `processMemory()` - so that the number of parallel workers and the map-reduce chunk sizes can be adjusted to the memory at hand</span>
* {{< bi square-half >}}<span>Runtime awareness: Add functions to **parallelly** for querying the wall-time budget and how much of it remains - `availableTime()` and `remainingTime()` - so that futures and map-reduce chunk sizes can be adapted to the time left, e.g. when running under a time-limited HPC job scheduler</span>
...
* {{< bi square >}}<span>**[future.clustermq]** - a backend to resolve futures on an HPC cluster via the **clustermq** package [[future#FR204], [future#FR267]]</span>
...
* {{< bi square-half >}}<span>Migrate to permissive license Apache License (>= 2), where possible.  Most packages are migrated, including **future**, **doFuture**, **futurize**, **listenv**, **progressr**, and **progressify**.  Remaining are **globals**, **parallelly**, **future.apply**, and **future.batchtools**, most of which include code from other packages and may need dual licensing.</span>
:::
...
[Resource Specifications]: roadmap/resources.html
