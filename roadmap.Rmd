...
Below are some^[This list is manual curated and may not be comprehensive.  Please see the issue trackers of the different package repositories for a more up-to-date view.] of the bigger milestones on the roadmap.  The issues linked that are examples of feature requests from end-users are prefixed 'FR', which will be resolved by the other non-FR issues.
...
* Restarting of failed or terminated futures, e.g. `reset(f)` and `v <- value(f)`.  Also automatic restarting of futures when parallel worker dies
  [[future#FR188], [future#205], [parallelly#32]]
...
* **future.aws.lambda** - a backend to resolve futures via the Amazon AWS Lambda.
  We have working group of several people actively working on this
  [[future#FR423]]

* **future.aws.batch** - a backend to resolve futures via the Amazon AWS Batch.
  We have working group of several people actively working on this
  [[future#FR423]]

* **future.google.functions** - a backend to resolve futures via the Google Cloud Functions.
...
* **future.sparklyr** - a backend to resolve futures in Spark via the **sparkly** package
  [[future#286]]
...
[future#286]: https://github.com/HenrikBengtsson/future/issues/286
