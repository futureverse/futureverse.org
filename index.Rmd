...
<div>
<img src="images/future.20200115.300dpi.png" style="width: 15ex; padding-left: 2ex; padding-bottom: 1ex; float: right;"/> 
...
Just like with sequential R code, _output, messages, warnings, and errors works as expected_ and can be handled using traditional R techniques - regardless how the code is parallelized.

It is designed so that you as a developer to _stay with your favorite coding style_, may it be base R or tidyverse, e.g. if you like base R `lapply()` there is a corresponding `future_lapply()` in the **future.apply** package and if you like tidyverse **purrr** `map()` there is a corresponding `future_map()` in the **furrr** package.  If you prefer `foreach()` from **foreach**, then **doFuture** provides a backend adapter.
...
Regardless how you use futures in your code, the user can with a single setting switch from running your code sequentially to running it in parallel on their local computer, across multiple machines on their local area network, in the cloud, or distributed on an high-performance compute cluster.
</div>
